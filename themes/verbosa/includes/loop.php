	...
	if ($verbosa_meta_author) {
		echo sprintf(
			'<span class="author vcard"' . cryout_schema_microdata('author', 0) . '>
				<i class="icon-pen icon-metas" title="'.__("Categories", "verbosa").'"></i>
...
function verbosa_set_featured_thumb() {
	...
	$featured_image = cryout_post_first_image( $post->ID );
	...
	if ( function_exists("has_post_thumbnail") && has_post_thumbnail() && $verbosas['verbosa_fpost']) {
		...
		// nothing to do, already got image from inside post
	...
	if ( !empty($featured_image) ): ?>
		<div class="post-thumbnail-container">
			<?php if (is_single() || is_page()) { ?>
				...
				<a <?php cryout_echo_bgimage($featured_image, 'post-featured-image') ?>> </a>
				<a><img class="post-featured-image" alt="" itemprop="image" src="<?php echo $featured_image; ?>" /></a>

			<?php }

			else { ?>
				<a href="<?php echo esc_url( get_permalink( $post->ID ) ) ?>" title="<?php echo esc_attr( get_post_field( 'post_title', $post->ID ) ) ?>"
					<?php cryout_echo_bgimage($featured_image, 'post-featured-image') ?>>
				...
				<a class="responsive-featured-image" href="<?php echo esc_url( get_permalink( $post->ID ) ) ?>" title="<?php echo esc_attr( get_post_field( 'post_title', $post->ID ) ) ?>">
					<img class="post-featured-image" alt="" itemprop="image" src="<?php echo esc_url( $featured_image ) ?>" />
				</a>
			<?php } ?>
